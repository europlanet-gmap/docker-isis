...
GISPY Dockerfile is accompanied by a `.txt` or `.yml` file, those are
...
If you want to build gispy image with different (Python) packages, or
different version, for example, you just have to change the `txt/yml`
files.

The version of  `isis` and `stereo-pipeline` will be taken from the [`.env`](/.env).
*There* is where you should change/fix the versions of ISIS and ASP you want to use.
